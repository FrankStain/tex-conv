...
namespace dll {	
	const char g_gyu_name[] = "Basic GYU converter";
	const char g_gyu_desc[] = "Basic converter for obfruscated and encrypted BMP files";

	plugin::logger_t* g_logger = NULL;
	
	gyu_desc_t::gyu_desc_t(){
	...
	const char* gyu_desc_t::name() const {
		return g_gyu_name;
	};

	const char* gyu_desc_t::desc() const {
		return g_gyu_desc;
	};
	
	const int gyu_desc_t::operators_count() const {
		return 1;
	};

	plugin::operator_t* gyu_desc_t::get_operator( const int index ){
		return ( index )? NULL : &m_operator;
	};

	void gyu_desc_t::set_logger( plugin::logger_t* logger_inst ){
