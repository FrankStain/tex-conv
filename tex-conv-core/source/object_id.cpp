...
objid_t::objid_t() : m_id(0) {

};

objid_t::objid_t( const string& name_id ){
	m_id = g_table.get_id( name_id );
};

const string& objid_t::name() const {
	return g_table.get_name( m_id );
};

uint32_t calc_crc32( const char* str );
...
inline uint32_t make_crc32( const uint32_t crc, const char c ){
	return ( crc >> 8 ) ^ crc32_table[ 0xFF & ( crc ^ c ) ];
};

inline uint32_t calc_crc32( const char* str ){
	uint32_t res = 0xFFFFFFFF;
	while( *str ){
		res = make_crc32( res, *str );
		str++;
	};
